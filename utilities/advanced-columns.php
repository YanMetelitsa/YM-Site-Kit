...
class YMSK_Advanced_Columns_Utility extends YMSK_Utility {
	/**
	 * Retrieves list table head thumbnail item.
	 * 
	 * @return array
	 */
	public static function get_thumbnail_th () : array {
		return [
			'ymsk-thumbnail' => sprintf( '<span class="text">%s</span> <span class="dashicons dashicons-format-image"></span>',
	...
	/**
	 * Retrieves list table body thumbnail item.
	 * 
	 * @param int $post_id Post ID.
	 */
	public static function get_thumbnail_td ( int $post_id ) {
	...
	'callback'    => function () {
		...
		add_filter( 'manage_post_posts_columns', function ( array $columns ) : array {
			return isset( $columns[ 'cb' ] ) ? [
				'cb'         => $columns[ 'cb' ],
				...YMSK_Advanced_Columns_Utility::get_thumbnail_th(),
				'title'      => $columns[ 'title' ],
		...
		add_action( 'manage_post_posts_custom_column' , function ( string $column, int $post_id ) {
			switch ( $column ) {
				case 'ymsk-thumbnail':
					YMSK_Advanced_Columns_Utility::get_thumbnail_td( $post_id );
					break;
			}
		}, 10, 2 );
